<template>
    <div class="ww-input-basic" :class="{ editing: isEditing }">
        <input v-if="content.type !== 'textarea'" :key="'ww-input-basic-' + step" ref="input"
            v-bind="{ ...$attrs, ...(wwElementState.props.attributes || {}) }" :value="value"
            class="ww-input-basic__input" :class="{
                editing: isEditing,
                hideArrows: content.hideArrows && inputType === 'number',
                'date-placeholder': content.type === 'date' && !value,
                '-readonly': isReadonly,
            }" :type="inputType" :name="wwElementState.name" :readonly="isReadonly" :required="content.required"
            :autocomplete="content.autocomplete ? 'on' : 'off'"
            :placeholder="isAdvancedPlaceholder ? '' : wwLang.getText(content.placeholder)" :style="style" :min="min"
            :max="max" :step="stepAttribute" @input="handleManualInput($event)" @blur="onBlur($event)"
            @focus="isReallyFocused = true" />
        <textarea v-else ref="input" v-bind="{ ...$attrs, ...(wwElementState.props.attributes || {}) }" :value="value"
            class="ww-input-basic__input" :class="{ editing: isEditing }" :type="content.type"
            :name="wwElementState.name" :readonly="isReadonly" :required="content.required"
            :placeholder="isAdvancedPlaceholder ? '' : wwLang.getText(content.placeholder)"
            :style="[style, { resize: content.resize ? '' : 'none' }]" :rows="content.rows"
            @input="handleManualInput($event)" @focus="isReallyFocused = true" @blur="isReallyFocused = false" />
        <div v-if="isAdvancedPlaceholder" ref="placeholder" class="ww-input-basic__placeholder"
            :class="{ editing: isEditing }" :style="placeholderSyle" @click="focusInput">
            <wwElement style="pointerevents: none" v-bind="content.placeholderElement"
                :states="value === 0 || (value && value.length) ? ['active'] : []"
                :ww-props="{ text: wwLang.getText(content.placeholder) }"></wwElement>
        </div>
    </div>
</template>
